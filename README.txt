...
DELIVERY FEE
------------
Open js/products.js and find "deliveryFee" near the top (currently 5.99).
Every order is local delivery (there's no collection option), so this
flat fee is added automatically to every order total. Change the number
to adjust it, or set it to 0 to turn it off.
...
2. The order is emailed to you instantly with a unique order number
   (e.g. WS-260706-K3P9).
3. The customer sees a PayPal button for the total, and is
