...
/* Delivery fee — every order is local delivery, so this always applies.
   Tiered by number of items in the basket: 1 item = €2, 2 or more items = €3 flat (capped). */
...
function grandTotal() {
  return basketTotal() + currentDeliveryFee();
}
...
function renderBasket() {
  ...
  var feeRow = document.getElementById("basket-delivery-fee");
  var fee = currentDeliveryFee();
  if (feeRow) {
    feeRow.hidden = fee <= 0;
    var feeValue = document.getElementById("basket-delivery-fee-value");
    if (feeValue) feeValue.textContent = money(fee);
  }

  document.getElementById("basket-total-value").textContent = money(grandTotal());
...
function orderSummaryText() {
  ...
  var fee = currentDeliveryFee();
  if (fee > 0) lines.push("Delivery fee: " + money(fee));
...
/* ---------- Checkout: confirm details, pay, THEN send order email ---------- */
var form = document.getElementById("order-form");
var errBox = document.getElementById("order-error");
var paymentStep = document.getElementById("payment-step");
var paymentErrBox = document.getElementById("payment-error");

var pendingOrder = null; // holds the order details between "Continue to payment" and "I've paid"
...
form.addEventListener("submit", function (e) {
  ...
  // Snapshot everything now — the basket/form shouldn't be touched again
  // until payment is confirmed and the order email has actually gone out.
  pendingOrder = {
    ref: ref,
    total: total,
    payload: {
      ...
      "Delivery fee": money(currentDeliveryFee()),
      "Total": money(total),
      "Name": form.name.value,
      "Email": form.email.value,
      "Phone": form.phone.value,
      "Address": form.address.value,
      "Delivery": form.fulfilment.value,
      "Paying by": form.payment_method.value,
      "Notes": form.note.value || "—"
    }
  };

  showPaymentStep(ref, total);
});

function showPaymentStep(ref, total) {
  form.hidden = true;
  paymentErrBox.hidden = true;
...
/* Only once the customer confirms they've paid do we actually send the order */
...
confirmBtn.addEventListener("click", function () {
  ...
  fetch("https://formsubmit.co/ajax/" + STORE_SETTINGS.orderEmail, {
    ...
    .then(function () {
      showSuccess(pendingOrder.ref, pendingOrder.total);
      pendingOrder = null;
    })
    .catch(function () {
      paymentErrBox.textContent = "Sorry — we couldn't send your order. Please check your connection and try again, or call us (your payment is safe either way).";
      paymentErrBox.hidden = false;
    })
    .finally(function () {
      confirmBtn.disabled = false;
      confirmBtn.textContent = "I've paid — send my order";
    });
});
...
function showSuccess(ref, total) {
  ...
  document.getElementById("order-ref-final").textContent = ref;
